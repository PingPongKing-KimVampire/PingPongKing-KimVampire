...
class PingpongRenderer {
	...
	_setGameSizeInfo() {
		...
		this.paddleHeightPercent = (this.clientInfo.gameInfo.sizeInfo.paddleHeight / this.boardHeight) * 100;
		this.paddleWidthPercent = (this.clientInfo.gameInfo.sizeInfo.paddleWidth / this.boardWidth) * 100;
	...
	_setPlayers(teamLeftList, teamRightList) {
		...
		for (const { clientId, clientNickname } of teamLeftList) {
			const player = {
				id: clientId,
		...
		for (const { clientId, clientNickname } of teamRightList) {
			const player = {
				id: clientId,
				nickName: clientNickname,
				team: "right",
